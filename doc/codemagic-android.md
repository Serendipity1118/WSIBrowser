...
| 9. 実機インストール | 完了 (テスター登録とオプトインが必要だった) |

### 9. 実機にインストールする (手動)

内部テストにビルドが上がっただけでは誰もインストールできない。テスターの登録が要る。

1. Play Console → WSI Browser → テストとリリース → テスト → 内部テスト
2. 上部の「テスター数」タブ (「リリース」の隣)
3. メールリストを新規作成し、**実機の Play ストアにログインしている Google アカウント**を追加
4. そのリストにチェックを入れ、画面下部の「変更を保存」を押す
5. 同じタブの下にある「テストのリンク」のオプトイン URL を実機で開き、「テスターになる」
6. 表示される Google Play へのリンクからインストール

登録したアドレスと端末の Play ストアのアカウントが違うと、オプトイン URL は
`App not available / Your account hasn't yet been invited` になる。
反映に数分かかることがある。

現在のテスター: serendip.takayanagi@gmail.com (Play Console のデベロッパーアカウントと同じ)。
iOS の TestFlight は develop@serendipy.jp で、**プラットフォームで別アカウント**になっている。
